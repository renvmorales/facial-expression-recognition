# Stochastic GD optimization for ANN models using 'relu' activation 
# function for multiclass problems.
...
class ANN_relu(object):
	...
	def fit(self, X, Y, alpha=1e-6, reg=1e-4, epochs=5000, 
		...
		for i in range(epochs):  # optimization loop
			slct = np.random.choice(N)
			PY = self.forward(X[slct,:].reshape(1,D))
			J[i] = cross_entropy_multi2(Trgt[slct,:].reshape(1,K), PY)
			self.back_prop(Trgt[slct,:].reshape(1,K), PY, alpha, reg)
			if i % 100 == 0:
		...
		if show_fig: 
			...
			plt.title('Evolution of the Cost through a Stochastic GD optimization     Total runtime: {:.3f} min'.format(self.elapsed_t)+'    Final Accuracy: {:.3f}'.format(np.mean(Y==self.predict(X))))
			plt.xlabel('Epochs')
			plt.ylabel('Cost')
			plt.legend()
			plt.show()
...
# fits the model with the hyperparameters set	
	model.fit(X, Y, alpha=1e-3, reg=0.9, epochs=8000, show_fig=True)
