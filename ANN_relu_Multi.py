...
class ANN_relu(object):
	...
	def fit(self, X, Y, alpha=1e-6, reg=1e-4, epochs=5000, 
		...
		if show_fig:
			...
			plt.title('Evolution of the Cost through a Full GA optimization   Total runtime: {:.3f} min'.format((end-start)/60)+'   Final Accuracy: {:.3f}'.format(np.mean(Y==np.argmax(PY,axis=1))))
			plt.xlabel('Epochs')
