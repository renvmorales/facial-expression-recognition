...
#  - Batch GD optimization using plain momentum and RMSprop
# 
...
class ANN_relu(object):
	...
	def fit(self, X, Y, alpha=1e-4, reg=1e-4, mu=0.8, epochs=5000, 
		...
		if show_fig: 
			...
			plt.title('Evolution of the Cost through a RMSprop Momentum-batch GD optimization     Total runtime: {:.3f} min'.format(self.elapsed_t)+'    Final Accuracy: {:.3f}'.format(np.mean(Y==self.predict(X))))
			plt.xlabel('Epochs')
			plt.ylabel('Cost')
			plt.legend()
			plt.show()
	...
	# updating weights using plain momentum + RMSprop!
	def back_prop(self, Y, PY, alpha, reg, mu, decay):
		...
		for i in range(1,len(self.W)+1):
			...
			self.W[-i] += self.dW[-i]
			self.db[-i] = mu*self.db[-i] - alpha*grad_b/(np.sqrt(self.cache_b[-i])+eps)
			self.b[-i] += self.db[-i]
...
# fit the model with the hyperparameters set	
	model.fit(X, Y, alpha=1e-5, epochs=5000, reg=0, mu=.9, show_fig=True)
