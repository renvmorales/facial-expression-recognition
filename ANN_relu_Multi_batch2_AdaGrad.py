...
#  - Batch GD optimization using plain momentum and AdaGrad 
# 
...
class ANN_relu(object):
	...
	def fit(self, X, Y, alpha=1e-3, reg=1e-4, mu=0.8, epochs=5000, 
		...
		if show_fig: 
			...
			plt.title('Evolution of the Cost through a AdaGrad Momentum batch GD optimization     Total runtime: {:.3f} min'.format(self.elapsed_t)+'    Final Accuracy: {:.3f}'.format(np.mean(Y==self.predict(X))))
			plt.xlabel('Epochs')
			plt.ylabel('Cost')
			plt.legend()
			plt.show()
...
# fit the model with the hyperparameters set	
	model.fit(X, Y, alpha=1e-3, epochs=5000, reg=0, mu=0.9, show_fig=True)
